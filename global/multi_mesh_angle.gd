...
func setup() -> void:
	...
	# Workaround for using primitive shapes.
	# Convert primitive to ArrayMesh.
	var arrMesh: ArrayMesh = ArrayMesh.new()
	arrMesh.add_surface_from_arrays(Mesh.PRIMITIVE_TRIANGLES, surface.mesh.get_mesh_arrays())
